  ...
  return (
    <div className='content'>
        ...
        <div className='list-content'>     
            ...
            {foodsList.map((item1,index)=>{
                return (index===activeIndex
                        &&
                        < div key={index}>
                        <div className='cate-title' >{item1.name}</div>
                            {foodsList[activeIndex].foods.map((item2)=>{
                                return <FoodItem key={item2.id} {...item2}/>
                            })}
                        </div>)
                //           {item1.foods.map((item2)=>{
                //            return <FoodItem key={item2.id} {...item2}/> 
                // return (<div key={item1.tag}>
                //           <div className='cate-title'>{item1.name}</div>
                //           {item1.foods.map((item2)=>{
                //            return <FoodItem key={item2.id} {...item2}/>
                //           })}
